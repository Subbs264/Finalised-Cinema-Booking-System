...
def update_details(name, cust_type, phone, performance, seats_str, price_paid):
    result = credential_connect('bookings', "SELECT * FROM bookings WHERE phone=?", fetch_all=True, params=(phone,))
    ...
    if result:
        ...
        credential_connect('bookings', "UPDATE bookings SET seats_booked=?, price_paid=? WHERE phone=?", fetch_all=False, params=(total_seats, existing_price, phone))
    else:
