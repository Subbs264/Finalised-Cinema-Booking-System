...
class Stage(ctk.CTk):
    ...
    def seating(self):
        '''Creates seats using a nested for loop, new rows are created after 10 seats A1, A2, A3...'''
        self.seats = {}
        self.seat_state = {}
        column_count = 0
    ...
    def book_all_button(self):
        self.book_button = ctk.CTkButton(self, text='Please book a seat', width=150, height=50, bg_color='gray', fg_color='gray', font=('Arial', 20), corner_radius=0, command=self.book_booking)
    ...
    def book_booking(self):
        ...
        if has_gap:
            self.error.configure(text=f'Seat {seat} is left out (Because its a middle seat), please include the middle seat or change your booking')
            return
